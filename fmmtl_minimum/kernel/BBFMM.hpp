 ...
 * K(t,s) = 1 / s-t
 */
#include "kernel/Util/constexpr_math.hpp"
...
constexpr T meta_pow(const T base, const unsigned exponent) {
  return (exponent == 0) ? 1 : base * constmath::pow(base, exponent-1);
}
 ...
 */
template <unsigned P, class _Kernel_Ty_>
class BBFMM
    : public fmmtl::Expansion<_Kernel_Ty_, BBFMM<P,_Kernel_Ty_> >
{
 public:
  ...
  struct MLData {
    point_type extents;
    constexpr static const std::size_t temp = meta_pow(P, D);
    std::array<charge_type,temp> data;
  };
  ...
  // Base class type
  using super_t = fmmtl::Expansion<_Kernel_Ty_, BBFMM<P,_Kernel_Ty_> >;
