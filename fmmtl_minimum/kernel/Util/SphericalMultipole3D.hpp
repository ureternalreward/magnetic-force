  ...
  inline static
    ...
    for (int n = 0; n != P; ++n) {
      for (int m = 0; m <= n; ++m, ++nm) {
...
#ifdef _NEW_M2L
        auto L = Ltarget[nm];
        auto Mj = Msource.begin();
#else
        auto & L = Ltarget[nm];
#endif
        complex_type* Wjn = W + (n - 1)*(n) / 2;
        for (int j = 0; j != P; ++j) {
...
#endif
#ifdef _NEW_M2L
          int neg1powm = neg1pow(m);

          //meven==true->neg1powm==1

          register double temp_[2];
          temp_[0] = Mj[0].real()*Wjn[m].real() - Mj[0].imag()*Wjn[m].imag();
          temp_[1] = -Mj[0].real()*Wjn[m].imag() - Mj[0].imag()*Wjn[m].real();

          //__m128d k0reg = _mm_set_pd(temp_[1], temp_[0]);

          real_type Ra[2];
          real_type Ia[2];
          for (int k = 1; k <= j; k++) {
            //store Mj[k] into register to save memory latency
            ...
            if (!(m & 1)) {
              Ia[0] = Ra[0] = (Wjn[m + k]).real();
              Ia[1] = Ra[1] = (Wjn[m + k]).imag();
            }
            else {
              Ia[0] = Ra[0] = -(Wjn[m + k]).real();
              Ia[1] = Ra[1] = -(Wjn[m + k]).imag();
            ...
            real_type ri_result[2];
            /*ri_result[0] = Rrea*reb - Rima*imb;
            ri_result[1] = -Iima*reb - Irea*imb;*/

            /*ri_result[0] = Ra[0]* Mj[k].real() - Ra[1]* Mj[k].imag();
            ri_result[1] = -Ia[1]* Mj[k].real() - Ia[0]* Mj[k].imag();*/
            ...
            ri_result[0] = Ra[0] - Ra[1];
            ri_result[1] = -Ia[1] - Ia[0];
#endif
            /*L.real(L.real() + ri_re
            sult[0]);ult[1]);
            //L += complex_type{ real_result ,imag_result };
            //L[0] += real_result;
            //L[1] += imag_result;
            //L += complex_type{ real_result ,imag_result };
            }
            L.imag(L.imag() + ri_result[1]);*/
#ifdef _NEW_M2L
            L.real(L.real() + ri_result[0]);
            L.imag(L.imag() + ri_result[1]);
          }
#endif
          //end for k

          //_mm_store_pd(temp_, k0reg);
#ifdef _NEW_M2L
          if (!(m & 1)) {
            L.real(L.real() + temp_[0]);
            L.imag(L.imag() + temp_[1]);
          }
          else {
            L.real(L.real() - temp_[0]);
            L.imag(L.imag() - temp_[1]);
        ...
        //end for j
#ifdef _NEW_M2L
        Ltarget[nm].real(L.real());
        Ltarget[nm].imag(L.imag());
#endif
      }
      //end for m
    }
