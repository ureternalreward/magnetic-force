...
class S2T_Compressed {
 public:
  FMMTL_IMPORT_KERNEL_TRAITS(_Kernel_Ty_);
  ...
  void execute(Context& c) {
    return execute(c.kernel(),
                   ...
                   c.result_near_begin(), c.result_near_end());
  }

  template <typename ChargeIter, typename ResultIter>
  void execute(const _Kernel_Ty_& K,
  ...
  static void execute(const _Kernel_Ty_& K,
  ...
  make(SourceRangeIter srfirst, SourceRangeIter srlast,
    ...
    return new S2T_Compressed<_Kernel_Ty_>(target_ranges,
                                      target_ptr,
                                      source_ranges,
                                      sources,
                                      targets);
  ...
  make(Context& c,
    ...
    for (unsigned k = 0; k < t_boxes.size(); ++k) {
      ...
      target_ranges.push_back(upair(unsigned(std::distance(first_target, c.target_begin(t))),
                                    unsigned(std::distance(first_target, c.target_end(t)))));

      const std::vector<source_box_type>& s_list = s_boxes[t.index()];
      for (unsigned n = 0; n < s_list.size(); ++n) {
        const source_box_type& s = s_list[n];
        source_ranges.push_back(upair(unsigned(std::distance(first_source, c.source_begin(s))),
                                      unsigned(std::distance(first_source, c.source_end(s)))));
      }
...
#pragma message("cuda!!!!!!!!!!!!!!!!!!!")
#include "fmmtl/dispatch/S2T/S2T_Compressed_cu_multiple_GPU.inl"
