...
#include "fmmtl/dispatch/S2T/S2T_Blocked_CSR_cuda_big_smem_small_blocksize.inl"
...
struct Data {
  ...
  Data(unsigned s, unsigned t, unsigned b, unsigned max_bpb)
      : num_sources(s),
        num_targets(t),
        num_blocks(b),
...
S2T_Compressed<_Kernel_Ty_>::S2T_Compressed(
  ...
  data_ = new Data(sources.size(), targets.size(), target_ranges.size(),max_body_per_box);
}
...
void S2T_Compressed<_Kernel_Ty_>::execute(
  ...
  blocked_p2p<256><<<num_blocks, data->max_body_per_box >>>(
      K,
      target_ranges_,
